...
> ✨ **AI features are optional and off by default.** Turn on **AI Features** in the editor to unlock categorisation, duplicate detection and long-press item/task lookups — they need a conversation agent such as Google Gemini. With AI off, the card still works fully as a plain shopping/todo list.
...
AI features are **off by default** — the card works fully without them as a plain shopping/todo list. Enable them with the **AI Features** toggle in the editor, then set up a conversation agent — **Google Gemini** is recommended. Each AI-powered feature also has its own toggle so you only turn on what you want.

**Setup:** Install the **Google Generative AI** integration in HA, select **`gemini-2.0-flash`** as the model, ensure the **Generative Language API** is enabled in [Google Cloud Console](https://console.cloud.google.com), then turn on **AI Features** and select **Google AI Conversation** as the AI Agent in the card's visual editor. Other conversation agents (Claude, OpenAI, Home Assistant's built-in AI, Ollama, etc.) may also work, but Gemini is recommended and best-tested.

**Free tier limits (gemini-2.0-flash):** 1,500 requests/day · 15 requests/minute — sufficient for normal daily use, since AI responses are cached per item.
